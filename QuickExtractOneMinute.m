...
minSectionLength=100; %Minimum length of walking section that must be within the window (in samples)
...
%     acc=interp1(x, acc, xq);
    ...
    for minute=1:numMinutes
        ...
        for num=testStart(numTest):(testCount+testStart(numTest)-1)
            ...
            Metrics(num, 4:5)=[length(acc)/Fs, length(acc)/(Fs*60)];
...
%%
    ...
    Metrics=MetricsTMP;
    clear MetricsTMP
    save([Patient '_' Dates{indDates} '_' Test{numTest} '_Minute' num2str(minute) '_Metrics.mat'], 'Metrics')
    end
    end
end
