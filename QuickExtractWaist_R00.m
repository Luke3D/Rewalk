...
MetricsPath = './MetricsData/Patients/R00/'; %folder where Avg Results per test are saved
MetricsPathDet = './MetricsData/Patients/R00/Detailed/'; %folder for each walk section results
datapathTT = './TestTimes/Patients/R00/'; %datapath of TestTimes Data
datapathacc = './TestTimes/Patients/R00/Rawaccdata/'; %datapath of raw acc data
...
for indDates = 1:length(filenames)
    ...
    %   (Should match format RXX_Waist(2014-01-01)RAW.csv and RXX_Ankle(2014-01-01)RAW.csv)
    file = [datapathacc Patient '_Waist(2014-' Date(6:end) ')RAW.csv'];
    ...
    for numTest=1:numOfTests
        ...
        numMinutes=(indend-indstart)/(Fs1*60);
        %%
        ...
        for num=testStart(numTest):(testCount+testStart(numTest)-1)
            ...
            % t1 = ceil(Fs/Fs1*TestTimes{num,3}); t2 = ceil(Fs/Fs1*TestTimes{num,4});
            t1 = ceil(TestTimes{num,3}); t2 = ceil(TestTimes{num,4});
