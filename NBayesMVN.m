%Compute Features for NaiveBayes Model using MVN
% clear all
%Features
% 1. Step Frequency
...
clearvars -except IpMulti

%Features to use
%[STEP FREQ, STD PHI, ENERGY. WALKING TIME RATIO, NUMBER OF STEPS,..,6mWT DIST (m)]
Features = [1 2 3 6];
D = length(Features);   %dimensions of the MVN
datapath = './MetricsData/NaiveBayes/Healthy30Hz/';

%Load all clips and weighted mean
load(strcat(datapath,'MetricsPost.mat'))    %Features from wmean across clips
load(strcat(datapath,'MetricsPost_NoWMean')); %Features from all clips

%Weighted mean features (one row per subject)
X = MetricsPost(:,Features);
% X([2 7],:) = [];    %remove outlier subject
Nsubj = size(X,1);

Xclips = [FeaturesPostAll(:,Features) FeaturesPostAll(:,end)];   %Features from all clips
...
for s = 1:Nsubj
    ...
    mui = nanmean(Xclips(Xclips(:,end) ~= s,1:end-1));
    covi = cov(Xclips(Xclips(:,end) ~= s,1:end-1));
    
%     mui = nanmean(X(ind,:));
%     covi = cov(X(ind,:));
  
    Xs = X(s,:);        %features of subj s            
    logPh(s) = -log( (2*pi)^(D/2)*sqrt(det(covi)) ) - 0.5*( (Xs-mui)*(covi\(Xs-mui)'))    %GNB of MVN
...
boxplot(I), title('z-score z_{\psi}')
set(gca,'FontSize',16)
...
muH = nanmean(Xclips(:,1:end-1));
covH = cov(Xclips(:,1:end-1));
...
%     end
    ...
    logPp(s) = -log( (2*pi)^(D/2)*sqrt(det(covH)) ) - 0.5*( (Xps-muH)*(covH\(Xps-muH)'))    %GNB of MVN
